...
if 'lr_data' in st.session_state:
    ...
    def get_receipt_html(copy_title):
        ...
        return f"""
        <div class="copy-box" style="border: 2px solid #000; padding: 6px; margin-bottom: 12px; background: #fff; font-family: Arial, sans-serif; font-size: 11.5px; box-sizing: border-box; min-height: 275mm;">
            <div style="float: right; font-weight: bold; font-size: 11px; border: 1.5px solid #000; padding: 2px 8px; background: #e0e0e0;">{copy_title}</div>
            
            <div style="background: #000; color: #fff; text-align: center; font-weight: bold; font-size: 10.5px; padding: 3px 0; margin-bottom: 6px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="width: 44%; vertical-align: top;">
                        <h2 style="margin: 0; font-size: 21px; font-weight: 900; letter-spacing: 1.5px; line-height: 1.1; font-family: 'Arial Black', Impact, sans-serif; transform: scaleY(1.15); transform-origin: top left;">FORTUNE EXPRESS CARGO</h2>
                        <i style="font-size: 10.5px;">Always on Time</i><br>
                        <b>E-mail :</b> fortuneexpresscargo@gmail.com<br>
                        <b>M. :</b> 9173165886
                    </td>
                    <td style="width: 32%; vertical-align: top; font-size: 10.5px; line-height: 1.3;">
                    ...
                    <td style="width: 24%; vertical-align: top; text-align: center;">
                        <div style="border: 1.5px solid #000; padding: 4px;">
                            <span style="font-size: 9.5px; font-weight: bold;">DOCKET NUMBER</span>
                            <div style="font-size: 20px; font-weight: bold; color: #d32f2f; line-height: 1;">{d['lr_no']}</div>
                            <div style="font-size: 10.5px;"><b>DATE:</b> {d['date']}</div>
                        </div>
                        <table style="width: 100%; border: 1px solid #000; margin-top: 3px; font-size: 10.5px; text-align: left;">
                            <tr><td style="border: 1px solid #000; padding: 2px 4px;"><b>FROM:</b> {d['from']}</td></tr>
                            <tr><td style="border: 1px solid #000; padding: 2px 4px;"><b>TO:</b> {d['to']}</td></tr>
                        </table>
                    </td>
                </tr>
            </table>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 6px; border: 1.5px solid #000; font-size: 10.5px; text-align: center;">
                <tr style="background: #e0e0e0; font-weight: bold;">
                    <td style="border: 1px solid #000; width: 35%; padding: 4px;" colspan="4">PACKAGE INFORMATION</td>
                    <td style="border: 1px solid #000; width: 25%; padding: 4px;" colspan="2">IN RUPEES</td>
                ...
                <tr style="font-weight: bold;">
                    <td style="border: 1px solid #000; padding: 4px;">TYPE OF PKG</td>
                    <td style="border: 1px solid #000; padding: 4px;">NO OF PKG</td>
                    <td style="border: 1px solid #000; padding: 4px;">VOLUME</td>
                    <td style="border: 1px solid #000; padding: 4px;">TOTAL PKG</td>
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">BASIC FREIGHT</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{d['basic_freight']:.2f}</td>
                    <td style="border: 1px solid #000; font-size: 18px;" rowspan="7">{billing_mark}</td>
                    <td style="border: 1px solid #000; font-size: 18px; color: green;" rowspan="7">{paid_mark}</td>
                    <td style="border: 1px solid #000; font-size: 18px; color: red;" rowspan="7">{topay_mark}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #000; padding: 4px;">{d['pkg_type']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">{d['no_pkg']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">{d['volume']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">{d['total_pkg']}</td>
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">VAL. SURCHARGE/DOCKET</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{(d['val_surcharge'] + d['docket_chg']):.2f}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #000; font-weight: bold; padding: 4px;" colspan="4">INVOICE DETAILS</td>
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">OTHER/ODA/SURCHARGE</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{(d['other_chg'] + d['oda_chg'] + d['surcharges']):.2f}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #000; padding: 4px;">INV: {d['inv_no']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">VAL: {d['inv_val']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">ACT WT: {d['act_wt']}</td>
                    <td style="border: 1px solid #000; padding: 4px;">CHG WT: {d['chg_wt']}</td>
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">CGST ({d['cgst']}) / SGST ({d['sgst']})</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{(d['cgst'] + d['sgst']):.2f}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #000; font-size: 8.5px; text-align: left; padding: 4px;" colspan="4" rowspan="3">
                    ...
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">IGST</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{d['igst']:.2f}</td>
                </tr>
                <tr style="font-weight: bold; background: #e0e0e0;">
                    <td style="border: 1px solid #000; text-align: left; padding: 4px;">GRAND TOTAL</td>
                    <td style="border: 1px solid #000; text-align: right; padding: 4px;">{d['grand_total']:.2f}</td>
                </tr>
            </table>

            <div style="font-size: 8.5px; margin-top: 5px; text-align: justify; line-height: 1.25;">
            ...
            <div style="background: #000; color: #fff; text-align: center; font-weight: bold; font-size: 10px; margin-top: 4px; padding: 3px 0;">
    ...
    <body style="margin:0; padding:0; background:#f5f5f5;">
        <div style="text-align: center; margin: 15px 0;" class="print-btn">
            <button onclick="window.print()" style="background-color: #28a745; color: white; padding: 12px 25px; border: none; font-size: 16px; font-weight: bold; border-radius: 5px; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                🖨️ DIRECT PRINT 3 COPIES (FULL PAGE A4)
            </button>
        </div>
        <div style="max-width: 900px; margin: 0 auto; background: #fff; padding: 5px;">
